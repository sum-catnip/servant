...
python_capability::python_capability( 
    ...
    : capability(name, py::str(capability.attr("__name__"))), m_pycapability(capability) { }

python_capability::python_capability(
    const std::string& name, py::function capability,
    const std::vector<std::shared_ptr<parameter>> params)
    : capability(name, py::str(capability.attr("__name__")), params), m_pycapability(capability) { }
...
result python_capability::execute(json::value& params) {
    ...
    try {
        ...
        return result(result::type::ERROR, "something really weird happened");
    }
}

py::function python_capability::pycapability() { return m_pycapability; }